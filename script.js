...
const isChineseVersion = document.documentElement.lang === 'zh-CN';
...
const translations = {
  en: {
    ...
    sleepReminder: '🌙 Go to Bed !',
    sleepIn: 'Sleep in',
    taskDesc: 'Add a description for this task',
    substepInput: 'Add sub-step',
    addStep: 'Add Step',
    urgent: 'Urgent',
    important: 'Important',
    etaPlaceholder: 'ETA(min)',
    fastTrackEmpty: 'Minimize time spent here. Done is better than perfect.'
  },
  zh: {
    greetings: {
      morning: '早上好！☀️',
    ...
    urgent: '紧急',
    important: '重要',
    etaPlaceholder: '预计时长(分钟)',
    fastTrackEmpty: '快速搞定，不要追求完美。'
  }
};

const t = translations[isChineseVersion ? 'zh' : 'en'];
...
/**
 * Initialize and start the guided tour for new users
 */
function initGuidedTour(force = false) {
  if (typeof introJs !== 'function') return;
  // Check if user has already completed the tour
  const tourCompleted = localStorage.getItem(TOUR_COMPLETED_KEY);
  if (!force && tourCompleted === 'true') return;

  // Define tour steps based on language
  const tourSteps = isChineseVersion
    ? [
        {
          element: '#todo-form',
          title: '➕ 添加任务',
          intro: '在这里添加你的主要任务。不用担心细节，我们稍后会处理。'
        },
        {
          element: '#list-container',
          title: '✅ 分解任务',
          intro: '把任务分解成小的、能获得多巴胺的子任务。'
        },
        {
          element: '.matrix-grid',
          title: '📊 分类任务',
          intro: '拖动或分配任务到四象限，看看什么真正紧急。'
        },
        {
          element: '#quick-memo-pad',
          title: '📝 随时记录',
          intro: '把你的灵感和奇怪的想法写在这里，这样你就不会忘记它们。'
        },
        {
          element: '#daily-habits-widget',
          title: '📅 每日打卡',
          intro: '在这里记录你的日常习惯，保持节奏更容易。'
        },
        {
    ...
    : [
        {
          element: '#todo-form',
          title: '➕ Add Tasks',
          intro: 'Add your main tasks here.<br>Don\'t worry about the details yet.'
        },
        {
          element: '#list-container',
          title: '✅ Break It Down',
          intro: '⏳ Enter Deadline & ETA<br>📍 Mark task as Urgent/Important<br> 🎯 Break into tiny steps <br> ⏰ Use the timer '
        },
        {
          element: '.matrix-grid',
          title: '📊 Categorize',
          intro: 'Drag or assign tasks here to see what\'s actually urgent.'
        },
        {
          element: '#quick-memo-pad',
          title: '📝 Brain Dump',
          intro: 'Brain-dump your intrusive thoughts here so you don\'t forget them.'
        },
        {
          element: '#daily-habits-widget',
          title: '📅 Daily Habits',
          intro: 'Track your daily habits here to stay consistent.'
        },
        {
          element: '#theme-switcher',
          title: '🎨 Pick Your Vibe',
          intro: 'Themes + language switch here.'
        }
      ];

  // Create and start intro.js tour
  const intro = introJs();
  intro.setOptions({
    steps: tourSteps,
    showProgress: true,
    ...
    autoPosition: true,
    prevLabel: isChineseVersion ? '上一步' : 'Previous',
    nextLabel: isChineseVersion ? '下一步' : 'Next',
    skipLabel: isChineseVersion ? '跳过' : 'Skip',
    doneLabel: isChineseVersion ? '完成' : 'Done'
  });
  ...
  intro.onexit(() => {
    // Also mark as completed if user exits early
    localStorage.setItem(TOUR_COMPLETED_KEY, 'true');
  });

  intro.onbeforechange(() => {
    const stepIndex = typeof intro._currentStep === 'number' ? intro._currentStep : 0;
    const shouldDisableScroll = stepIndex === 1 || stepIndex === 2;
    intro.setOption('scrollToElement', !shouldDisableScroll);
  });

  // Start the tour
  intro.start();

  // Fix skip button size by directly manipulating DOM
  setTimeout(() => {
    const skipBtn = document.querySelector('.introjs-skipbutton');
    if (skipBtn) {
      skipBtn.style.cssText = 'display: flex !important; justify-content: center !important; align-items: center !important; width: 90px !important; height: 36px !important; padding: 0 !important; font-size: 14px !important; line-height: 1 !important; margin: 0 auto !important; border: none !important;';
    }
  }, 100);
}
...
function serializeTask(li) {
  const isTutorial = li.classList.contains('tutorial-task') || li.dataset.isTutorial === 'true';
  return {
    ...
    isTutorial,
    tutorialPlaceholderTitle: isTutorial ? (li.dataset.tutorialPlaceholderTitle || li.querySelector('.task-text')?.textContent || '') : '',
    tutorialPlaceholderDesc: isTutorial ? (li.dataset.tutorialPlaceholderDesc || li.querySelector('.task-desc')?.placeholder || '') : ''
  };
}
...
function loadTasks() {
  ...
  // Parse existing tasks or start with empty array
  if (raw) {
    try {
  ...
  // If list is empty, create a tutorial task
  if (tasks.length === 0) {
    ...
    tasks.push({
      ...
      quadrant: 'do-first',
      isTutorial: true,
      tutorialPlaceholderTitle: isChineseVersion ? '任务名称 (' : 'Task Name (Double-Click to Edit)',
      tutorialPlaceholderDesc: isChineseVersion ? '欢迎！这是一个示例任务。您可以设置截止日期、追踪时间并将任务分解为小步骤。' : 'Welcome! This is a sample task. You can set deadlines, track time, and break tasks into small steps.',
      substeps: [{
        id: 'tutorial-step-' + Date.now(),
        text: isChineseVersion ? '✓ 标记完成此步骤以查看进度。' : '✓ Check off this step to see progress.',
        completed: false,
        elapsedMs: '0'
      }],
      completed: false,
  ...
  tasks.forEach(task => {
    const isTutorialTask = task.isTutorial || (typeof task.id === 'string' && task.id.startsWith('tutorial-'));
    const tutorialTitle = isChineseVersion ? '任务名称 (双击修改)' : 'Task Name (double-click to edit)';
    const tutorialDesc = isChineseVersion
      ? '欢迎！这是一个示例任务。您可以设置截止日期、追踪时间并将任务分解为小步骤。'
      : 'Welcome! This is a sample task. You can set deadlines, track time, and break tasks into small steps.';
    const taskTextForRender = isTutorialTask ? '' : (task.text || 'Untitled');
    const taskDescForRender = isTutorialTask ? '' : (task.desc || '');

    const li = createTaskElement(taskTextForRender, task.priority || 'medium', task.deadline || '');
    ...
    // Add tutorial class if this is a tutorial task
    if (isTutorialTask) {
      li.classList.add('tutorial-task');
      li.dataset.isTutorial = 'true';
      li.dataset.tutorialPlaceholderTitle = tutorialTitle;
      li.dataset.tutorialPlaceholderDesc = tutorialDesc;
      // Set placeholder text for tutorial task
      const taskText = li.querySelector('.task-text');
      if (taskText) {
        taskText.textContent = task.tutorialPlaceholderTitle || tutorialTitle;
        taskText.setAttribute('data-placeholder', 'true');
      }
    }

    const desc = li.querySelector('.task-desc');
    if (desc) {
      desc.value = taskDescForRender;
      // For tutorial task, set placeholder instead of value
      if (isTutorialTask) {
        desc.value = '';
        desc.placeholder = task.tutorialPlaceholderDesc || tutorialDesc;
      }
    }
...
function startTimer(el, card) {
  ...
  // Find the parent task (if this is a substep)
  const parentLi = el.classList.contains('substep') ? el.closest('li.task-item') : null;
  const parentCard = parentLi ? parentLi.querySelector('.task-timer-panel') : null;
  ...
  el._timerInterval = setInterval(() => {
    ...
    // If this is a substep, also update parent task timer
    if (parentLi) {
      let parentElapsed = parseInt(parentLi.dataset.elapsedMs || '0', 10);
      parentElapsed += 1000;
      parentLi.dataset.elapsedMs = String(parentElapsed);
      updateTimerDisplay(parentLi, parentCard, parentElapsed);
    }
  }, 1000);
...
function attachSubstepHandlers(subLi, parentLi) {
  ...
  /**
   * Substep text click handler: Allow inline editing of substep name
   */
  const substepText = subLi.querySelector('.substep-text');
  if (substepText) {
    substepText.addEventListener('click', function (e) {
      e.stopPropagation();
      startInlineEditSubstep(subLi, substepText, cb, parentLi);
    });
  }
}
...
/**
 * Inline edit helper for substeps: replace the substep text element with an input for editing.
 * Supports Enter to save, Escape to cancel, and blur to save.
 */
function startInlineEditSubstep(subLi, substepSpan, checkbox, parentLi) {
  // If an editor already exists, do not create another
  if (subLi.querySelector('.substep-edit-input')) return;

  const original = substepSpan.textContent;
  const input = document.createElement('input');
  input.type = 'text';
  input.className = 'substep-edit-input';
  input.value = original;

  // Insert input before the span and hide the original span
  substepSpan.parentNode.insertBefore(input, substepSpan);
  ...
  function finish(save) {
    ...
    if (save) {
      const val = input.value.trim();
      if (!val) {
        alert('Substep name cannot be empty');
        input.focus();
        done = false; // allow further editing
        return;
      }
      substepSpan.textContent = val;
      // Editing implies active work; clear completed state
      subLi.classList.remove('completed');
      checkbox.checked = false;
      if (parentLi) updateSubstepSummary(parentLi);
    }
    input.remove();
    substepSpan.style.display = '';
  }
...
function createTaskElement(taskText, priority = 'medium', deadline = '') {
  ...
  li.innerHTML = `
    <div class="task-main">
      <div class="task-left">
        ...
        <div class="task-controls">
          ...
          <div class="eta-group">
            <input type="number" class="eta-input" min="0" placeholder="${t.etaPlaceholder}" aria-label="ETA in minutes">
    ...
    <textarea class="task-desc" rows="1" placeholder="${t.taskDesc}"></textarea>

    <div class="substeps">
      <input type="text" class="substep-input" placeholder="${t.substepInput}">
      <button type="button" class="add-substep-btn">${t.addStep}</button>
...
function attachTaskEventHandlers(li) {
  ...
  // Initialize urgent/important checkboxes based on data attributes
  if (urgentToggle) urgentToggle.checked = li.dataset.isUrgent === 'true';
  if (importantToggle) importantToggle.checked = li.dataset.isImportant === 'true';
...
/**
 * Reset all user data including tasks, quick notes, and daily habits
...
function resetAllData() {
  ...
  // Clear quick notes
  const quickMemoItems = document.querySelectorAll('.quick-memo-item');
  quickMemoItems.forEach((item, index) => {
    if (index === 0) {
      item.textContent = '';
      item.setAttribute('data-placeholder', isChineseVersion ? '在此输入笔记...' : 'Quick notes...');
    } else {
      item.textContent = '';
    }
  });

  // Clear daily habits inputs
  const habitInputs = document.querySelectorAll('.habit-input');
  habitInputs.forEach(input => {
    input.value = '';
    if (input.classList.contains('sleep-input')) {
      input.placeholder = isChineseVersion ? '23:00' : '11 PM';
    }
  });

  // Uncheck all habit checkboxes
  ...
  // Reload tasks to regenerate the tutorial task if list is empty
  loadTasks();

  // Show success message
  alert(isChineseVersion ? '✓ 所有数据已清除！' : '✓ All data cleared successfully!');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tourBtn) {
    tourBtn.addEventListener('click', () => initGuidedTour(true));
  }
